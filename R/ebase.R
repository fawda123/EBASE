...
#' registerDoParallel(cl)
#'   
#' res <- ebase(dat, interval = 900, H = 1.85, progress = TRUE, n.chains = 2, bprior = c(0.2, 0.1))
#'
#' stopCluster(cl)
#' }
ebase <- function(dat, H, interval, ndays = 1, aprior = c(0.2, 0.1), rprior = c(20, 5), bprior = c(0.251, 0.005), interp = TRUE, maxgap = 1e6, n.iter = 10000, update.chains = TRUE, n.burnin = n.iter*0.5, n.chains = 3, n.thin = 10, progress = FALSE, model_file = NULL){
  ...
  # process
  output <- foreach(i = grps, .packages = c('here', 'R2jags', 'rjags'), .export = c('nstepd', 'metab_update', 'interval', 'aprior', 'rprior', 'bprior')
                                                                         ) %dopar% {
    ...
    # Set
    dat.list <- list("num.measurements", "nstepd", "interval", "aprior", "rprior", "bprior", "DO_obs", "PAR", "DO_sat", "sc", "H", "U10")
